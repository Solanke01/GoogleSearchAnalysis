...
def main():
    ...
    if keywords_input:
        ...
        st.subheader('Advertising Suggestions')
        st.write("Based on the analysis of Google search trends, here are some advertising suggestions:")
        for kw in kw_list:
            st.write(f"- Target Google Ads campaigns using the keyword '{kw}' to reach users actively searching for related products or information.")
            st.write(f"- Leverage social media advertising on platforms like Facebook and Instagram to target users interested in '{kw}'.")
            st.write(f"- Collaborate with influencers or content creators in the '{kw}' niche to promote products/services.")
            st.write(f"- Develop targeted email marketing campaigns focused on '{kw}' to engage potential customers.")
